...
| `PollCard.vue` | Summary card for a poll shown in community/home feed. Links to `PollDetailPage`. Shows live vote counts and issuer trust badge (`Issuer linked`/`Unverified identity`) when real-name authorship is enabled. | `poll: Poll` |
| `PostCard.vue` | Summary card for a community post. Shows title, author pseudonym, vote score, comment count, image thumbnail, and issuer trust badge (`Issuer linked`/`Unverified identity`) when real-name authorship is enabled. | `post: Post` |
| `CommentCard.vue` | Single comment with author pseudonym, vote controls, nested replies, and issuer trust badge (`Issuer linked`/`Unverified identity`) when real-name authorship is enabled. | `comment`, `postId` |
