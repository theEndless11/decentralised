...
| File | Class | Purpose |
|---|---|---|
| `pollService.ts` | `PollService` | Poll CRUD, invite code generation/validation, vote recording in GunDB. Schnorr-signs polls on create (`authorPubkey`, `contentSignature`). Community/all-poll subscriptions use hydration-first loading with batch processing plus soft/hard timeouts to avoid startup hangs while preventing startup content from being treated as "new". `loadPoll()` remains API-first for fast reads, but its API/index calls now follow the shared runtime relay config (`config.relay.api`) instead of a hardcoded hostname. Cold-start `loadPoll()` / `loadPollOptions()` then fall back to Gun cache/live sync and finally the gun-relay MySQL search endpoint (`/db/search?prefix=v2/polls/{id}`) to warm Gun data. Encrypts poll content (question, options, description) via `EncryptionService`/`KeyVaultService` when community has an encryption key; `decryptPoll()` reverses at read time. Also calls `indexForSearch()` to push data to relay for full-text search. |
